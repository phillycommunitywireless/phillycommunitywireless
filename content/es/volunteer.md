---
title: "Hazte voluntario con PCW"
header_photo: /images/web updates/Group_Solar_HQ.jpg
header_alt: "12 personas sentadas alrededor de una mesa en un interior con computadoras portátiles y equipo tecnológico sobre la mesa."

segments: 
  - template: call-to-action-photo-right
    class: bg-light-gray
    title: "Inscríbete"
    text: "Nos constituimos como una coalición de base dirigida por voluntarios, y nuestro dedicado equipo de personal incorpora regularmente a nuevos voluntarios del área de Filadelfia para ayudar a construir y mantener nuestra red. ¡Te invitamos a unirte y participar en nuestra organización!


Si está interesado en participar, rellene nuestro [formulario de inscripción para voluntarios].(https://tally.so/r/w2ODaj). "
    photo: "/images/pcwvolunteers2.jpg"
    photo_alt_text: "7 personas posando para una foto grupal en un tejado con un dispositivo y árboles altos detrás de ellas."
    button: 
      button_text: "Inscríbete como voluntario"
      button_href: "https://tally.so/r/w2ODaj"
    # no_button_arrow: "true"

  - template: text-lr
    left:
        title: "Técnico"
        text: "
        Ofrece tu ayuda como voluntario para tareas técnicas con PCW, que incluyen la instalación de equipos en hogares y azoteas de toda Filadelfia, la asistencia a cursos de formación presenciales en Norris Square, la configuración de redes, la cartografía y la visualización de datos, y el desarrollo web.
        
        
        Muchos de nosotros estamos aprendiendo estas habilidades, por lo que, si son nuevas para ti, recibirás formación en un entorno colaborativo. Si tienes experiencia, agradecemos tus conocimientos y tu apoyo en la formación.
        
        
        Estas son algunas de las áreas de habilidades relevantes para nuestro trabajo:

        * Instalación: acceso a tejados, subir escaleras, perforar ladrillos, fijar cables

        * Hardware y cableado: terminación y prueba de cables CAT5e, montaje de equipos en mampostería

        * Redes: diseño de redes, seguridad, protocolos, hardware, malla, Linux/UNIX

        * Desarrollo de software

        * SIG y cartografía, visualización de datos, ciencia de datos

        * UX/UI, diseño gráfico

        * Documentación técnica
        "
    right:
        title: "Comunicación y participación comunitaria"
        text: "
        Ofrece tus servicios como voluntario para realizar tareas de comunicación y participación comunitaria con PCW, lo que incluye participar en eventos comunitarios, hacer campaña y repartir volantes, marketing y diseño gráfico, entre otras cosas. Si hablas español, agradeceremos tu apoyo con las traducciones y la participación comunitaria en general.


        Los voluntarios que participan en las actividades de divulgación de PCW también están invitados a unirse a las instalaciones, que son una gran oportunidad para aprender más sobre el trabajo de PCW, interactuar con los miembros de la comunidad que nos abren sus hogares y espacios, y llevar a cabo actividades de divulgación después de la instalación.
        
        
        ## Estas son algunas de las áreas de habilidades relevantes para nuestro trabajo:
        
        * Campañas electorales y reparto de volantes

        * Eventos: organización de mesas, planificación y gestión

        * Marketing y redes sociales

        * Diseño gráfico: carteles, postales, volantes, merchandising, camisetas

        * Documentación: fotos, videos y textos

        * Investigación y formación educativa
        "
  - template: eventcalendar
    heading: "Próximos eventos"
    text: "¡Consulta nuestro calendario para hacerte una idea de los próximos eventos y oportunidades de voluntariado presenciales! La mayoría de nuestras instalaciones se coordinan los jueves o viernes, y nuestro objetivo es organizar sesiones de voluntariado los sábados una o dos veces al mes."
---

¡Nos encantan los voluntarios! Utiliza tus habilidades (¡sea cuales sean!) para contribuir a la comunidad de Filadelfia y apoyar el acceso a Internet para todos. Hay oportunidades de voluntariado presencial y a distancia disponibles; te invitamos a participar en la medida que te resulta más conveniente.


Si deseas apoyar nuestro trabajo pero no puedes ser voluntario en este momento, consulta nuestra [página de apoyo](https://phillycommunitywireless.org/support) para conocer otras formas de ayudar.
